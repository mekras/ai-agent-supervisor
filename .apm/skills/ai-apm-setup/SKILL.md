...
актуальный `apm.yml`, переносимый детерминированный контроль качества
`apm run tests` без вызовов модели и сети, отдельный опциональный модельный
прогон `apm run evals`
...
4. Прочитай существующие `apm.yml`, `README.md`, списки `.apm/skills/**`,
   ...
   `references/apm-collection-settings.md`. Если задача доставляет оснастку в
   целевой проект, дополнительно прочитай `references/tooling-installers.md`.
