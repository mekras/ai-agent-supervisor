...
7а. Не оставляй в публикуемом каталоге навыка `__pycache__`, `.pyc` или `.pyo`.
   Перед проверкой выполни поиск таких артефактов. Для запуска Python-проверок
   установи `PYTHONDONTWRITEBYTECODE=1`, но не подменяй им итоговую проверку
   состава.
7б. Для каждого публичного Python-скрипта P1 первого уровня в `scripts/`
   ...
   Для каждого скрипта нужен хотя бы один успешный рабочий сценарий. Сопоставь
   команды из процедуры навыка со сценариями контракта. Скрипт, который создаёт
   или изменяет состояние, обязан успешно пройти этот путь и сохранить его в
   проверяемом внешнем формате. Для JSON проверь повторное чтение созданного
   файла. Подключи
