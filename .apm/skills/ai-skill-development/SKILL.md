...
7а. Не оставляй в публикуемом каталоге навыка `__pycache__`, `.pyc` или `.pyo`.
   Перед проверкой выполни поиск таких артефактов. До запуска Python-файла из
   установленной копии навыка или поставляемой оснастки добавь к интерпретатору
   ключ `-B` либо передай `PYTHONDONTWRITEBYTECODE=1`. Делай это и при разовом
   запуске теста или диагностики, а не удаляй созданный кэш после того, как
   `apm audit` обнаружил дрейф. Эта мера не заменяет итоговую проверку состава.
