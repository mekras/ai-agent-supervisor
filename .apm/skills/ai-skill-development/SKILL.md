...
7б. Для каждого публичного Python-скрипта P1 первого уровня в `scripts/`
   ...
   результат, а не только внутреннюю функцию или `--help`. Скрипт, который
   создаёт или изменяет состояние, обязан сохранить его в проверяемом внешнем
   формате. Для JSON проверь повторное чтение созданного файла. Подключи
