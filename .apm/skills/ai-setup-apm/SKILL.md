...
До запуска Python-файла из установленной коллекции или её оснастки включай
запрет записи байткода в самом процессе: добавь ключ `-B` к команде
интерпретатора либо передай `PYTHONDONTWRITEBYTECODE=1`. Применяй это до
запуска, в том числе для разовой диагностики и тестов. Не создавай кэш с
расчётом удалить его после `apm audit`: такой файл уже создаёт дрейф
установленной копии навыка.
...
9. Для коллекции с исходниками в `.apm/skills` используй две команды P1.
   ...
   исходном дереве `.apm`. До запуска любой поставляемой Python-команды включай
   запрет записи байткода ключом `-B` либо переменной
   `PYTHONDONTWRITEBYTECODE=1`, но не считай эту меру заменой итоговой проверки
   публикуемого состава.
