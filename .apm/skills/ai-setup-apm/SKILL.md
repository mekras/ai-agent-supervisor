...
3а. Отдели публичный договор коллекции от контекста её разработки. Во всех
...
8. Раздели проверки на две цели и не смешивай их в одной команде:
   - `scripts.tests` — переносимый контроль качества. Только детерминированные
     ...
     её совпадение с источником), без вызовов модели, сети и внешнего
     инструмента командной строки. Этот контроль качества обязан выполняться
     без ошибок у стороннего участника и в непрерывной интеграции. Не помещай
     модельный прогон в `tests`: иначе участник без нужного окружения не сможет
...
9. До полного модельного прогона выбирай самый узкий достаточный цикл:
   ...
   навыков.
11. Для механического создания или обновления базового `apm.yml` можно
   применить `scripts/setup-apm-collection`. Установщик разбирает YAML и
   отказывается работать с дублированными ключами. После запуска проверь diff,
   разбери итоговый `apm.yml` средством YAML и повтори запуск установщика: в
   манифесте должны остаться по одному `scripts` и `includes`, а `.apm/skills/**`,
...
13. Перед завершением режима `setup` закрой явный чек-лист готовности:
   ...
   аудита. Для границы проверь всё поставляемое дерево, а не только `SKILL.md`:
...
```bash
python3 <путь-к-установленному-ai-setup-apm>/scripts/validate-apm-git-boundary.py
```

Валидатор читает `apm.lock.yaml`, берёт из `dependencies[*].deployed_files`
пути проекций каждого пакета-зависимости и сопоставляет их с `git ls-files`.
