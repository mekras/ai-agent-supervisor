...
9. Для коллекции с исходниками в `.apm/skills` используй две команды P1.
   ...
   Для каждого скрипта обязателен хотя бы один успешный рабочий сценарий.
   Команды из процедуры навыка должны быть сопоставлены со сценариями
   контракта. Для сохранённого JSON рабочий сценарий выполняет запись и
   повторно читает созданный файл.
   Контроль качества обязан отклонять `__pycache__`, `.pyc` и `.pyo` в
   исходном дереве `.apm`. Запускай поставляемые Python-проверки с
   `PYTHONDONTWRITEBYTECODE=1`, но не считай эту переменную заменой итоговой
   проверки публикуемого состава.
