...
2. Маршрутизируй задачу по типу:
   ...
   `references/tooling-installers.md`. Перед выбором цели манифеста,
   обновлением CLI или диагностикой сбоя установки, блокировки или выпуска
   также прочитай `references/known-version-issues.md`: часть ошибок вызвана
   ограничением конкретной версии `apm`, а не проектом.
...
5а. Если пакет публикует `.apm/instructions/**`, прочитай
   `references/instruction-scope.md` и выбери область каждой инструкции до её
   записи.
   Инструкция без условия области попадает в контекст с первого хода, а
   инструкция с условием разворачивается условным правилом и загружается лениво
   при обращении к подходящему файлу. Текст, который обязан действовать до
   первого касания файла, публикуй без условия области. Значение области,
   совпадающее со всеми файлами, не задавай: оно читается как «всегда», а
   работает как отложенная загрузка.
...
17а. Проверь отдельно, доходят ли развёрнутые правила до контекста агента.
   Правило с условием области не даёт ошибки, не попадает в расхождения
   установки и выглядит целым, но действует только при обращении к подходящему
   файлу. Раздели развёрнутые правила на условные и безусловные, сопоставь
   условие с назначением текста и подтверди вывод наблюдением, а не наличием
   файла. Процедура и признаки — в `references/instruction-scope.md`. Находку
   исправляй в исходнике пакета, а для чужого пакета назови пакет-владелец.
18. Если повтор установки или `apm audit --ci` сообщает, что выпуск обязан
   ...
   это дефект выпуска или его метаданных. Сверь симптом с
   `references/known-version-issues.md`: если он совпадает с записанным
   ограничением версии CLI, не лечи его правкой `apm.yml`, зависимостей или
   файла блокировки — сообщи затронутую версию, известный обход и апстримную
   ссылку. Если симптом не совпадает ни с одной записью, но воспроизводится на
   чистом состоянии и не объясняется проектом, добавь проверенную находку в
   справку по её формату. Сообщи пакет, версию и путь, сохрани
...
- Не принимай ограничение версии CLI из `references/known-version-issues.md`
  как подтверждённое для другой версии без собственной проверки; и не
  списывай непонятную ошибку на проект, если её симптом совпадает с записанным
  ограничением версии.
...
- Не задавай публикуемой инструкции условие области, совпадающее со всеми
  файлами: такая инструкция всё равно разворачивается условным правилом и
  загружается лениво, хотя читается как всегда действующая.
- Не считай установленное правило действующим только потому, что его файл на
  месте и его содержание верно. Не чини недоставленное правило правкой
  развёрнутой копии.
...
- как разобраны предупреждения `apm audit` по категориям `unintegrated`,
  `modified`, `orphaned`;
- какие развёрнутые правила доходят до контекста всегда, какие условны по путям
  и какие из условных не соответствуют назначению своего текста;
