...
11. Для механического создания нового базового `apm.yml` можно применить P1
   ...
   библиотеку и намеренно отказывается менять существующий YAML. Существующий
   манифест обновляй по процедуре P0, затем проверь diff и разбери итоговый файл
   доступным YAML-средством. В манифесте должны остаться по одному `scripts` и
   `includes`, а `.apm/skills/**`, `includes` и команда тестов должны
   соответствовать реальным файлам проекта.
