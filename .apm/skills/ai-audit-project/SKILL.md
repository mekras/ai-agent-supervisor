...
1. Запусти доступный `run-skill-script-contract-tests.py` для найденных корней
   навыков. Наличие контрактного файла само по себе не доказывает покрытие.
2. Для каждой операции из `operations` нужен успешный сценарий с её
   идентификатором в `covers` и совпадающим `command_prefix`. Ожидаемый отказ
   вспомогательной команды не подтверждает основной путь.
3. Сопоставь команды, прямо указанные в процедуре навыка, с `operations`
   контракта.
4. Если команда сохраняет JSON, требуй `expect.files` с `json: true`, повторное
   чтение созданного JSON и проверку ожидаемых данных состояния. Код возврата,
   `stdout` или наличие файла не заменяют эту проверку.
5. Если запускатель сообщил отсутствие успешного сценария, не предлагай
   исправить его успешным запуском другой команды. До формулирования находки
   найди в процедуре команду, которая выполняет непокрытый основной путь или
   создаёт состояние, и назови именно её в направлении исправления.

Пример достаточного исправления для команды создания состояния:
`"files": [{"path": "state.json", "json": true, "contains": "status"}]`.
В выводе аудита явно укажи, что запускатель повторно читает созданный файл как
JSON после успешного выполнения команды. В `acceptable_fix_direction` такой
находки обязательно назови успешный сценарий основной команды, `expect.files`
с `json: true`, повторное чтение JSON и проверку ожидаемых данных.
