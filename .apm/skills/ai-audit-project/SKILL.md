...
1. До запуска сценариев составь рабочую таблицу охвата для каждой публичной
   операции: скрипт, `command_prefix`, условные файлы и настройки, объявленные
   `inputs`, успешные фикстуры и проверяемый внешний результат. Строка нужна
   для каждой операции, включая операции без условных входов. Не переходи к
   выводу о полноте аудита, пока таблица не заполнена по коду скриптов, а не по
   одному содержимому контракта.
...
4. Успешный сценарий на минимальной фикстуре не доказывает ветви, которые
   ...
   которую минимальная фикстура прятала. Поле `inputs` обязательно для каждой
   операции. Пустой массив допустим только после проверки кода и означает, что
   условных входов у операции нет.
5. Только после заполнения таблицы запусти доступный
   `run-skill-script-contract-tests.py` для найденных корней навыков. Наличие и
   прохождение контрактного файла само по себе не доказывает покрытие. Перед
   завершением сопоставь результат запуска с таблицей. Пропущенная строка,
