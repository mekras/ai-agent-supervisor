...
1. Для Python-скриптов выполни доступный `run-skill-script-contract-tests.py`
   ...
   `evals/script-contract-tests.json`, отклоняет вырожденные объявленные входы
   и печатает по каждому скрипту очередь невыполненных исполняемых строк.
   Успешный код возврата без разбора этой очереди не доказывает
   работоспособность автоматизации.
2. Ошибку инструмента о вырожденном входе устраняй только новым успешным
   сценарием с содержательными данными: пустая коллекция не покрывает
   обработку элементов, одна запись — взаимодействие записей. Не добавляй ещё
   одну пустую фикстуру и не заменяй существующий сценарий другого класса
   входа без доказательства эквивалентности.
3. Каждый диапазон невыполненных строк прочитай в коде и классифицируй:
   обработка данных, защитный отказ, зависимость от внешней среды. Для ветви
   обработки данных добавь в `evals/script-contract-tests.json` успешный
   сценарий с фикстурой, которая активирует ветвь, и проверь значимые данные
   результата повторным чтением созданного файла, а не только код возврата,
   `stdout` или наличие файла. Для остальных классов зафиксируй короткое
   обоснование в отчёте аудита. Вспомогательный тест вне контракта не заменяет
   контрактный сценарий.
...
терминально, его ошибки не устранены, а каждый диапазон невыполненных строк не
получил активирующий сценарий либо названное в отчёте обоснование.
