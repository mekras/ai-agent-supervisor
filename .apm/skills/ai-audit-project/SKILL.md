...
2. До вывода о полноте запусти доступный
   ...
   сценариев к действительно отсутствующим путям внутри фикстур и исключает
   пути, которые сама операция затем создаёт или заменяет. Отрицательный
   результат проверки типа существующего пути не доказывает его отсутствие.
   Каждый оставшийся путь, не объявленный в `inputs` покрываемой операции,
   опровергает пустой массив. Перенеси найденные пути и результат команды в
   карту операций. Разбирай пути по одному и до изменения `inputs` проверь
   состояние исходной фикстуры: уже существующий путь нельзя объявлять
   отсутствующим условным входом только по сообщению трассировщика. Ошибочная
   классификация одного пути не отменяет остальные находки. Если используется
   старая версия запускателя без различения отсутствия, несовпадения типа и
   записываемого состояния, установи роль каждого пути по исходной фикстуре,
   коду и исполнению, исключи только подтверждённые ложные срабатывания и
   выходы, затем продолжи проверку остальных.
...
5. Дополни карту чтением кода: выпиши условные настройки и ветви, которые
   ...
   входом, затем повтори тот же сценарий после исправления. Не исправляй
   контракт добавлением в `inputs` обязательных файлов, каталогов или других
   путей, которые уже присутствовали в исходной успешной фикстуре. Проход после
   такого изменения не подтверждает исправление причины. Поле `inputs`
